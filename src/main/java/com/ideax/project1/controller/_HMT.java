...
public class _HMT {
	...
	private final String[][] SEARCH_ENGINE_LIST = { { "1", "baidu.com", "word|wd|w" }, { "2", "google.com", "q" },
			{ "4", "sogou.com", "query" }, { "6", "search.yahoo.com", "p" }, { "7", "yahoo.cn", "q" },
			{ "8", "soso.com", "w" }, { "11", "youdao.com", "q" }, { "12", "gougou.com", "search" },
			{ "13", "bing.com", "q" }, { "14", "so.com", "q" }, { "14", "so.360.cn", "q" }, { "15", "jike.com", "q" },
			{ "16", "qihoo.com", "kw" }, { "17", "etao.com", "q" }, { "18", "soku.com", "keyword" },
			{ "19", "easou.com", "q" }, { "20", "glb.uc.cn", "keyword|word|q" } };
		...
		if (referer == "" || (this.isIncludedInDomain(pathHost) && this.isIncludedInDomain(refererHost))) {
			...
			for (int i = 0; i < this.SEARCH_ENGINE_LIST.length; i++) {
				...
				if (m.find()) {
					...
					if (this.searchWord != "" || this.SEARCH_ENGINE_LIST[i][0] == "2"
							|| this.SEARCH_ENGINE_LIST[i][0] == "14" || this.SEARCH_ENGINE_LIST[i][0] == "17") {
	...
	private String getPixelUrl() {
		...
		String pixelUrl = "";
		try {
			pixelUrl = "http://hm.baidu.com/hm.gif" + "?si=" + this.siteId + "&et=" + this.eventType
					+ (this.eventProperty != "" ? "&ep=" + URLEncoder.encode(this.eventProperty, "UTF-8") : "")
					+ "&nv=" + isNewVisit + "&st=" + sourceType
					...
					+ (this.visitUrl != "" ? "&u=" + URLEncoder.encode(this.visitUrl, "UTF-8") : "") + "&v="
					+ this.VERSION + "&rnd=" + this.getRandomNumber();
	...
	public String trackEvent(String category, String action, String label, String value) {
		this.eventType = 4;
		this.eventProperty = this.replaceSpecialChars(category) + "*" + this.replaceSpecialChars(action)
